...
namespace CoherentNoise.Texturing
{
	...
	public static class TextureMaker
	{
        ...
        ///<returns></returns>
        /// 
        public static Texture Make(int noise_width, int noise_height, Func<float, float, Color> colorFunc, TextureFormat format = TextureFormat.RGB24)
        {
            Color[] cols = new Color[noise_width * noise_height];
            for (int ii = 0; ii < noise_width; ii++)
            {
                for (int jj = 0; jj < noise_height; jj++)
                {
                    cols[ii + jj * noise_width] = colorFunc((float)ii / noise_width, (float)jj / noise_height);
                }
            }
            var res = new Texture2D(noise_width, noise_height, format, false);
